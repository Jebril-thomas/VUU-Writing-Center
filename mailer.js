...
/**
 * Send staff notification when a new appointment request comes in.
 */
async function sendAppointmentNotificationToStaff(appt) {
  const transport = createTransport();
  const body = `
    <h2>New Appointment Request</h2>
    <p>A student has submitted an appointment request through the Writing Center website.</p>

    <div class="ref-box">
      <div class="ref-label">Reference Number</div>
      <div class="ref-number">${escHtml(appt.ref)}</div>
    </div>

    <table class="detail-table">
      <tr><td>Name</td><td>${escHtml(appt.name)}</td></tr>
      <tr><td>Email</td><td><a href="mailto:${escHtml(appt.email)}">${escHtml(appt.email)}</a></td></tr>
      <tr><td>Session Type</td><td>${escHtml(appt.session_type)}</td></tr>
      <tr><td>Preferred Date</td><td>${escHtml(appt.preferred_date)}</td></tr>
      <tr><td>Preferred Time</td><td>${escHtml(appt.preferred_time)}</td></tr>
      <tr><td>Submitted</td><td>${formatDate(appt.submitted_at)}</td></tr>
    </table>

    ${appt.message ? `
    <p><strong>Additional Notes:</strong></p>
    <p style="background:#F9FAFB;border-left:3px solid #C8922A;padding:14px 18px;border-radius:4px;white-space:pre-wrap;font-size:0.92rem">
      ${escHtml(appt.message)}
    </p>` : ''}

    <p><a href="mailto:${escHtml(appt.email)}?subject=Re: Your Appointment Request (Ref ${escHtml(appt.ref)})" class="cta-btn">
      Reply to ${escHtml(appt.name)} →
    </a></p>`;
  ...
  await transport.sendMail({
    ...
    to:      STAFF_EMAIL,
    replyTo: appt.email,
    subject: `[Appointment] ${appt.session_type} — ${appt.name} — ${appt.preferred_date} — Ref ${appt.ref}`,
    html:    wrap(body),
    text:    `New appointment request from ${appt.name} (${appt.email}).\nType: ${appt.session_type}\nDate: ${appt.preferred_date} at ${appt.preferred_time}\nRef: ${appt.ref}${appt.message ? '\n\nNotes: ' + appt.message : ''}`,
  });
}
