 ...
 * Also handles: scroll reveal · counter animation · bar animation · scroll tint
 */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  topbar.innerHTML = `
    <div class="container">
      <div class="topbar-inner">
        <p>Virginia Union University &nbsp;·&nbsp; Richmond, Virginia &nbsp;·&nbsp; Est. 1865</p>
        <p>Questions? <a href="contact.html">Contact us</a>&ensp;|&ensp;<a href="appointments.html">Book Now →</a></p>
      </div>
    </div>`;
  ...
  const drawerLinks = pages.filter(p => !p.cta).map(p => {
    const isActive = p.href === currentPage;
      ...
      <div class="nav-drawer" id="drawerPanel" role="dialog" aria-label="Navigation menu">
        ...
        <div class="nav-drawer-cta">
          ...
          <div class="nav-drawer-info">
            Free for all enrolled VUU students<br>Mon–Fri 9 AM–6 PM &nbsp;·&nbsp; Sat 10 AM–2 PM
          </div>
        </div>
      </div>
    </div>`;
      ...
      <div class="footer-top">
        ...
        <div class="footer-col">
          ...
          <div class="footer-hours">
            ...
            <p>Mon–Thu: 9 AM–7 PM<br>Fri: 9 AM–4 PM<br>Sat: 10 AM–2 PM<br><span style="opacity:0.5">Sunday: Closed</span></p>
          </div>
        </div>
      </div>
  ...
  function openDrawer() {
    ...
    setTimeout(() => { const fl = drawerPanel.querySelector('a'); if (fl) fl.focus(); }, 320);
  }
  ...
  // Focus trap
  drawerPanel && drawerPanel.addEventListener('keydown', e => {
    if (e.key !== 'Tab') return;
    const els = drawerPanel.querySelectorAll('a, button, [tabindex]:not([tabindex="-1"])');
  ...
  // Close on link click
  drawerPanel && drawerPanel.querySelectorAll('a').forEach(a => a.addEventListener('click', closeDrawer));
  ...
  if (statsEl) new IntersectionObserver(([e]) => { if (e.isIntersecting) runCounters(); }, { threshold: 0.4 }).observe(statsEl);

  /* ─── Progress bars ──────────────────────────────── */
  document.querySelectorAll('.hero-main-card').forEach(card => {
    new IntersectionObserver(([e]) => {
      if (e.isIntersecting) {
        card.querySelectorAll('.hmc-bar-fill').forEach((bar, i) => {
          bar.style.animationDelay = `${i * 220}ms`;
