...
const stmts = {
  ...
  insertAppointment: db.prepare(`
    INSERT INTO appointments
      (ref, name, email, session_type, preferred_date, preferred_time, message)
    VALUES
      (@ref, @name, @email, @session_type, @preferred_date, @preferred_time, @message)
  `),

  getAppointmentByRef: db.prepare('SELECT * FROM appointments WHERE ref = ?'),

  getAllAppointments: db.prepare(`
    SELECT id, ref, name, email, session_type, preferred_date, preferred_time, status, submitted_at
    FROM appointments ORDER BY submitted_at DESC
  `),
